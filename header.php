...
<link rel="stylesheet" type="text/css" href="<?php echo get_template_directory_uri();?>/segurosdigitales.css" />
...
<?php if(get_option('skypanel_broadcast_top_header_style') == 'Bright'){ echo '<link rel="stylesheet" type="text/css" href="'.get_template_directory_uri().'/scripts/css/styles/bright/top_header.css" />'; } //if user choices to show it bright show css. ?>

<?php //checks to see which style for the categories ?>
<?php if(get_option('skypanel_broadcast_categories_style') == 'Dark' ){echo '<link rel="stylesheet" type="text/css" href="'.get_template_directory_uri().'/scripts/css/styles/dark/categories.css" />';} ?>
...
elseif(get_option('skypanel_broadcast_featured_style') == 'Featured Short'){echo '<link rel="stylesheet" type="text/css" href="'.get_template_directory_uri().'/scripts/css/styles/bright/featured.css" />'; }
elseif(get_option('skypanel_broadcast_featured_style') == 'Featured Post' OR  $video_ == 'true'){echo '<link rel="stylesheet" type="text/css" href="'.get_template_directory_uri().'/scripts/css/styles/bright/featured_slider_post.css" />';}
}
?>

<?php //footer styling ?>

<?php if(get_option('skypanel_broadcast_footer_style') == 'Bright'){echo '<link rel="stylesheet" type="text/css" href="'.get_template_directory_uri().'/scripts/css/styles/bright/footer.css" />'; } ?>

<?php //checks to see which slider is selected so the proper css can be applied ?>
<?php if(get_option('skypanel_broadcast_featured_style') != 'Featured Long' AND get_option('skypanel_broadcast_featured_style') != 'Featured Post' AND  $video_ != 'true' AND get_option('skypanel_broadcast_featured_style') != '' ) { $featured_slider_style = 'featured_slider.css'; } elseif(get_option('skypanel_broadcast_featured_style') == 'Featured Long' AND $video_ != 'true' OR get_option('skypanel_broadcast_featured_style') == '') { $featured_slider_style = 'featured_long_slider.css'; } elseif(get_option('skypanel_broadcast_featured_style') == 'Featured Post' OR $video_ == 'true' ){ $featured_slider_style = 'featured_slider_post.css';} ?>
...
<style type="text/css">
/* Custom Css Colors */
.list_category h2{color:<?php echo get_option('skypanel_broadcast_headings_color'); ?> !important;}
.list_post h2{color:<?php echo get_option('skypanel_broadcast_headings_color'); ?> !important;}
#right h2{color:<?php echo get_option('skypanel_broadcast_headings_color'); ?> !important;}
.rightwidget h2 a{color:<?php echo get_option('skypanel_broadcast_headings_color'); ?> !important;}
a.cat_arrow{background:<?php echo get_option('skypanel_broadcast_headings_color'); ?> url('<?php echo get_template_directory_uri();?>/images/category_arrow.png')  !important;}
a:hover.cat_arrow{background:#1f1f1f url('<?php echo get_template_directory_uri();?>/images/category_arrow.png')  !important;}
.list_category h2 a{color:<?php echo get_option('skypanel_broadcast_headings_color'); ?> !important;}
h1.fontface {font: 53px 'ColaborateBoldRegular', Arial, sans-serif; color:#222222;}
#footer h5{color:<?php echo get_option('skypanel_broadcast_headings_color'); ?>; !important}
#left h1{color:<?php echo get_option('skypanel_broadcast_headings_color'); ?>; !important}
a.featured_button{background:<?php echo get_option('skypanel_broadcast_headings_color'); ?> !important;}
.list_post div.readm div.button a{background:<?php echo get_option('skypanel_broadcast_headings_color'); ?> !important;}
</style>
<?php } ?>
<?php wp_head(); ?>
...
<p<?php if(get_option('skypanel_broadcast_display_date') != 'Enabled') { echo ' class="hideobject"'; } ?>><?php echo date('l, M. j, Y'); ?></p>

<span class="timestamp<?php if(get_option('skypanel_broadcast_display_date') != 'Enabled') { echo ' hideobject';} ?>"><script type="text/javascript" language="JavaScript"><!--
user_time();
//--></script></span>

<ul<?php if(get_option('skypanel_broadcast_display_rss') != 'Enabled') { echo ' class="hideobject"';} ?>><li><a href="<?php echo home_url() .'?feed=rss2'; ?>"><img src="<?php echo get_template_directory_uri();?>/images/header-rss-icon.png" class="rss_icon" alt="<?php _e('Follow our feed', 'skyali'); ?>" /><?php _e('Subscribe to rss', 'skayli'); ?></a></li></ul>
...
<div id="header_container"<?php if(get_option('skypanel_broadcast_heading_pattern') == 'Disabled'){echo ' class="white_bg"';} ?>>
...
<?php if(get_option('skypanel_broadcast_spotlight') != 'Disabled' ){ include_once('includes/spotlight.php'); } ?>

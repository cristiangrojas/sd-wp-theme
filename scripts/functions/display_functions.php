...
function site_logo(){
	if(get_option('skypanel_broadcast_logo_settings') == 'Text Logo'){ echo '<h1 class="site_logo_text fontface">'.get_bloginfo('name').'</h1>'; } 
	else {echo '<a href="'.get_bloginfo('url').'">'; if(get_option('skypanel_broadcast_top_logo') != '')
  { echo '<img src="'.get_option('skypanel_broadcast_top_logo').'"  border="0" alt="'.get_bloginfo('name').'" class="logo"  />';}
  elseif(get_option('skypanel_broadcast_headings_color') != ''){echo '<img src="'.get_template_directory_uri().'/images/logo_custom_color.png" width="262" height="66" class="logo" alt="'.get_bloginfo('name').'"/>';}
...
function display_468x60(){
	$image_468 = get_option('skypanel_broadcast_image_url');
	$disable = '';
	if(get_option('skypanel_broadcast_ad_options') == 'Disabled'){
	$disable = 'hideobject';
	}
	echo '<div class="top_ad '.$disable.'">';
	$ad_code = get_option('skypanel_broadcast_ad_code');
	if(!empty($ad_code))
	{
	echo $ad_code;	
    }
    elseif(!empty($image_468)){
	echo '<a href="'.get_option('skypanel_broadcast_image_link_address').'"><img src="'.get_option('skypanel_broadcast_image_url').'" alt="468 X 60"  /></a>';
	}
	else{
...
function featured_option(){
	if(get_option('skypanel_broadcast_featured_options') == 'Disabled'){
...
function footer_widget_option(){
	if(get_option('skypanel_broadcast_footer_widget') == 'Disabled'){
		echo 'class="hideobject"';
	}
}
...
function other_categories_option(){
	if(get_option('skypanel_broadcast_other_categories') == 'Disabled'){
		echo 'hideobject';
	}
}
...
function exclude_top_navigation(){
	$exclude =  get_option('skypanel_broadcast_exclude_top_navigation_pages');
...
function check_exclude_top_navigation(){
	$check_exclude = get_option('skypanel_broadcast_exclude_top_navigation_pages');
...
function exclude_categories(){
	$exclude_c = get_option('skypanel_broadcast_exclude_categories');
...
function check_exclude_categories(){
	$check_exclude = get_option('skypanel_broadcast_exclude_categories');
	if(!empty($check_exclude)){
...
function disable_author(){
	if(get_option('skypanel_broadcast_about_author') == 'Disabled'){
		echo 'hideobject';
	}
}
...
function disable_related_news(){
	if(get_option('skypanel_broadcast_related_news') == 'Disabled'){
		echo 'hideobject';
	}
}
...
function disable_share_icons(){
	if(get_option('skypanel_broadcast_share_icons') == 'Disabled'){
		echo 'hideobject';
	}
}
...
function disable_blog_img(){
	if(get_option('skypanel_broadcast_featured_blog_post_image') == 'Disabled'){
		echo 'hideobject';
	}
}
